...
def process_registration(image_bytes: bytes, skip_liveness: bool = False) -> np.ndarray:
    ...
    if len(faces) == 0:
        ...
        gc.collect()
        raise ValueError(
            "No se detecto ningun rostro en la foto. Por favor, asegurese de que el rostro "
            "este de frente, bien iluminado (sin sombras fuertes ni contraluz) y libre de "
    ...
    if len(faces) > 1:
        ...
        gc.collect()
        raise ValueError(
            "Se detectaron varios rostros en la foto. Por favor, asegurese de que solo "
